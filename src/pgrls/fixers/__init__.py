...
def render_migration(
    fixes: list[Fix], *, tool_version: str, offline: bool = False
) -> str:
    ...
    When `offline=True`, the header is replaced with an offline
    caveat warning that the SQL was generated without access to
    the live catalog (BYPASSRLS roles, SECURITY DEFINER functions,
    FK context are unseen). When `offline=False` (the default, live
    path), the header is byte-identical to the pre-existing form.
    """
    count = len(fixes)
    plural = "fix" if count == 1 else "fixes"
    if offline:
        header = (
            f"-- Remediation SQL generated offline by pgrls {tool_version}.\n"
            "--\n"
            "-- Generated offline from --sql-file/--snapshot; NOT validated against\n"
            "-- live catalog state. BYPASSRLS roles, SECURITY DEFINER functions,\n"
            "-- FK context, triggers, and indexes are unseen — review carefully\n"
            f"-- before applying. {count} {plural}.\n"
        )
    else:
        header = (
