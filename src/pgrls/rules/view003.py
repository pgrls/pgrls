...
* Give the matview a per-tenant OWNER — the body runs as the OWNER at
  REFRESH regardless of who issues the command. Who may issue it is
  `has_privs_of_role`: the owner, an INHERIT member of it, or a
  superuser (measured — the superuser refresh above captured the
  owner's rows); an unrelated role gets
