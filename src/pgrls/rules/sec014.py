    ...
    def _violation(self, fn: SecdefFunction) -> Violation:
        return Violation(
            ...
            message=(
                ...
                "so any SELECT/INSERT/UPDATE/DELETE inside the body "
                "is checked against the OWNER's grants and RLS "
                "policies. That is a full bypass when the owner is "
                "RLS-exempt for the table (superuser, BYPASSRLS, or "
                "the table's owner while FORCE is off); otherwise "
                "the policies still run, but against the owner. "
                "Either way a role with EXECUTE on this function "
