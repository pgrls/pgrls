    ...
    def _violation(self, table: Table, trigger: Trigger) -> Violation:
        ...
        return Violation(
            ...
            message=(
                ...
                "function's owner — so the body is checked against THAT "
                "role's grants and policies. That is a full bypass only "
                "when the owner is RLS-exempt for the table (superuser, "
                "BYPASSRLS, or the table's owner while FORCE is off); "
                "otherwise the policies still run, but against the "
                "owner. Audit the function "
