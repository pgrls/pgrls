...
The README declares "Postgres 10+" as the supported floor. Without
...
PG16. A future commit that depends on a PG13+ catalog column or
function would silently break PG10/11/12 with no failing test.

This file declares the floor as a single source of truth and
asserts the connected Postgres meets it. Set
`PGRLS_TEST_PG_IMAGE=postgres:12-alpine` (or :11/:10) to verify
the floor is actually satisfied; the floor check below catches
the case where a future change tightens the implicit floor without
updating the README claim.
"""
...
# (`MMmmpp` — major*10000 + minor*100 + patch). PG10 is 100000,
# PG12 is 120000, PG16 is 160000. Update this constant — and the
# README — together when the floor moves.
_README_PG_FLOOR_NUM: int = 100000
