...
-- Both tables carry a RESTRICTIVE token policy plus a PERMISSIVE
-- companion (to postgres, not PUBLIC) so SEC009 (RLS enabled, no
...
CREATE POLICY sec002_clean_p ON public.sec002_clean
    AS RESTRICTIVE FOR SELECT TO PUBLIC USING (id > 0);
CREATE POLICY sec002_clean_permit ON public.sec002_clean
    FOR SELECT TO postgres USING (id > 0);
