...
def test_explain_format_html_catalog_renders_every_rule_row() -> None:
    ...
    # 66 rules ship today (catalog header should say so).
    assert "<strong>66</strong> rules" in result.output
...
def test_lint_fires_every_registered_rule_in_combined_fixture(
    ...
    try:
        ...
        runner = CliRunner()
        # SEC050 targets the Supabase `storage.objects` table, which lives in
        # the `storage` schema — outside lint's default `public`-only scan.
        # Scan it explicitly so the combined fixture exercises that rule too.
        result = runner.invoke(
            main, ["lint", "--database-url", pg_url, "--schemas", "public,storage"]
        )
        assert result.exit_code == 1, result.output
        ...
        for rule_loc in (
            ...
            "SEC042  public.allbad_sec042_rpc\n",
            "SEC043  public.allbad_sec043_child\n",
            "SEC044  public\n",
            ...
            "SEC052  public.allbad_sec052\n",
            "SEC053  public.allbad_sec053\n",
        ...
        apply_sql(
            ...
            # SEC053's postgres_fdw SERVER is cluster-global and survives the
            # schema drop (CASCADE also removes the foreign table, already gone
            # with public) — drop it so it can't leak into the clean-DB e2e test.
            "DROP SERVER IF EXISTS allbad_sec053_srv CASCADE; "
