...
Privileges and policy applicability follow **role membership** the way Postgres does: a grant to a group reaches its `INHERIT` members (a `NOINHERIT` member holds none of it), a policy `TO` a group applies to *every* member, and `pg_read_all_data` / `pg_write_all_data` confer `SELECT` / writes respectively. A role that owns a table — or inherits its owner — reads every row unless the table is `FORCE`d, and a superuser reads everything. Earlier versions matched role names literally and reported `DENIED` for a role that read every row in each of those three cases; each was measured on PG16.
