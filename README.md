...
- **`DENIED`** — no privilege on any path pgrls models, or RLS is on with no applicable permissive policy (Postgres default-denies).
- **`UNDECIDED`** — the rows can't be bounded: a materialized view over the table (its rows were captured at `REFRESH`), a foreign-key action (which rows it rewrites depends on the data), or two different filtered paths whose union is unknown. Treat it as possibly reachable, never as denied.
...
- A **`SECURITY DEFINER` trigger** runs as its owner for anyone who can fire it, which means writing its table, directly or through another door. There's no `EXECUTE` check: measured, a role holding only `INSERT` on one table emptied another through an `AFTER INSERT` trigger. A trigger function can't be called directly, so `EXECUTE` on one opens nothing.
- A **rewrite rule** runs its actions as the relation's owner for anyone who writes the relation (measured: `ON INSERT … DO ALSO DELETE FROM archive` emptied a table the inserting role could not touch). An `INSTEAD` rule on a view replaces the view's own auto-update.
...
A door whose SQL can't be traced is listed in its own section, with who can open it (a function's `EXECUTE` holders, or the write that fires a trigger or rule), instead of being attributed to a table. That covers dynamic SQL, another language, a call into a function pgrls can't see (including `query_to_xml`-style built-ins that run SQL), and statements such as `DO`, `CALL` or `SET search_path`. What *can* be traced is still credited. A `DENIED` cell does not rule out an untraced door.
...
**Columns.** By default the grid shows `PUBLIC`; `anon` and `authenticated` when they exist; and every other role whose reach differs from `PUBLIC`'s somewhere in the grid. That means a cell with a different verdict or predicate, or an untraced door `PUBLIC` can't open. A role that can do exactly what everyone can adds nothing. `--roles` fixes the columns; `--include-system-roles` also considers `pg_*` roles.
