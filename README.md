...
- **`OPEN`** — every row. The role holds the command's privilege and RLS lets every row through, either in its own session or through a door (below):
  - **the privilege** comes from a grant (its own, one it inherits, or `PUBLIC`'s), ownership, `pg_read_all_data` / `pg_write_all_data`, or superuser;
...
- **`COND`** — some rows. The predicate (in `--format json` / `html`) is the OR of the applicable permissive clauses AND-ed with the restrictive ones. A cell reached through a door names the door in its note and carries that door owner's predicate.
- **`DENIED`** — no privilege on any path, or RLS is on with no applicable permissive policy (Postgres default-denies).
- **`UNDECIDED`** — the rows can't be bounded: either a materialized view over the table (its rows were captured at `REFRESH`), or two different filtered paths whose union is unknown. Treat it as possibly reachable, never as denied.
...
**Doors.** A cell counts every way in, not only the role's own session, and a door counts for the command it runs:
...
- A function body that can't be traced (dynamic SQL, another language, a call into a function pgrls can't see) is listed in its own section instead of being attributed to a table. A `DENIED` cell does not rule such a function out.
...
Doors are found in every schema; the rows are the tables in `--schemas`.

A separate **sensitive columns** section lists, per role, the columns whose names match SEC045's patterns (`email`, `ssn`, `phone`, …) that the role can read, and the paths that reach each one.

**Columns.** By default the grid has a column for every role that reaches at least one table, plus `PUBLIC` and, when they exist, `anon` and `authenticated`. `--roles` fixes the columns; `--include-system-roles` also considers `pg_*` roles.

**What it does not model:**

- **`SET ROLE`.** Each column is a session running *as that role*. A `NOINHERIT` member that can switch to its group reads what the group's column shows.
...
- **Writes through `INSTEAD OF` triggers or rules.**
