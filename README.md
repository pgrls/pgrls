...
- **`UNVERIFIED`** — Z3 is unavailable, the predicate is outside the decidable fragment, the solver timed out, or (`cross-tenant` / `write`) the policy has no single `<column> = <session identity>` equality on an identity/discriminator column — SEC021's default name set, overridable with `[lint.rules.SEC021].identity_columns` in the `--config` file — to verify against. This is the point where the **verifier degrades to the linter** — no claim is made; run `pgrls lint` for the heuristic rules.
