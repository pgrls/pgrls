...
- **`DENIED`** — no privilege on any path pgrls models, or RLS is on with no applicable permissive policy (Postgres default-denies). For `TRUNCATE`, also a table that another table's foreign key references, when the role can't truncate that one too (it would need `CASCADE`).
- **`UNDECIDED`** — the rows can't be bounded:
  ...
  - two different filtered paths whose union is unknown.
...
- A **`SECURITY DEFINER` trigger** runs as its owner for anyone who can fire it, which means writing its table, directly or through another door. There's no `EXECUTE` check: measured, a role holding only `INSERT` on one table emptied another through an `AFTER INSERT` trigger.
  ...
  - A `BEFORE … FOR EACH ROW` trigger on `INSERT` fires before RLS checks the new row.
...
A body that sets any other parameter (`SET`, `set_config`, or its own `SET` clause) is traced, but its filtered doors are `UNDECIDED`.
...
- **Casts and built-in-looking operators.** A cast, or an operator that reuses a built-in symbol on its own type, backed by a user function is not detected.
