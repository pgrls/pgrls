...
- **`OPEN`** — every row is reachable: the role is granted the command's privilege **and** either RLS is off, the role is exempt from RLS (`BYPASSRLS` or superuser), or an applicable permissive policy is unconditionally `true` with no restrictive policy narrowing it.
...
- **`UNDECIDED`** — the answer turns on role memberships that were not captured (a grant the role may inherit, a policy it may fall under). Never reported as `DENIED`, which would be a guess in the unsafe direction. A live database always captures the graph, so this appears only for inputs without one.

Per command it evaluates the clause Postgres actually applies — `WITH CHECK` for INSERT, `USING` for SELECT/UPDATE/DELETE. (For UPDATE, v1 shows the read-side `USING` — *which rows are reachable*; the write-side `WITH CHECK` gate that validates the new row image is not modeled per-cell.) Columns default to `PUBLIC`, `anon`, `authenticated` plus every non-system role named by a grant or policy, or exempt from RLS (`BYPASSRLS` or superuser) (`--roles` overrides; `--include-system-roles` adds `pg_*`). Like `report`, it runs **no rules**.

Privileges and policy applicability follow **role membership** the way Postgres does: a grant to a group reaches its `INHERIT` members (a `NOINHERIT` member holds none of it), a policy `TO` a group binds the same `INHERIT` members (a `NOINHERIT` member is bound by neither a permissive nor a restrictive one), and `pg_read_all_data` / `pg_write_all_data` confer `SELECT` / writes respectively. A role that owns a table — or inherits its owner — reads every row unless the table is `FORCE`d, and a superuser reads everything. Earlier versions matched role names literally and reported `DENIED` for a role that read every row in each of those three cases; each was measured on PG16.

The `SELECT` column also counts **doors**: a definer view or a `SECURITY DEFINER` function the role can open runs as its *owner*, so it can widen a cell (never narrow one, and never touch a write command). An invoker view resets to the calling role and opens no door; a function whose body cannot be read is not attributed to any table.

A separate **sensitive columns** section lists, per role, the columns whose names match SEC045's patterns (`email`, `ssn`, `phone`, …) that the role can read, and the paths that reach each one. A column reached through a view is *possibly* reachable — column lineage through the view body is not traced, so the base table's sensitive columns are reported; that errs toward over-reporting, the safe direction.
