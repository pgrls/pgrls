...
- **`anon`** (default) — an **anonymous** session (every auth function — `auth.uid()`/`role()`/`jwt()`, `current_setting(...)` — returning `NULL`, the unauthenticated state) cannot read any row. A *leaking* predicate is gated on whether an anon session can actually invoke the policy — its roles reachable from `anon`/`PUBLIC` through the `pg_auth_members` closure — so a leak scoped to a role anon is not a member of (e.g. `TO authenticated`) exposes nothing and is **not** a leak, while one `TO` a role anon *is* granted (`GRANT app_reader TO anon`) still is. A *scoped* predicate (`tenant_id = auth.uid()`) proves isolated from the predicate alone (anon's `auth.uid()` is NULL → no match), independent of role. Honors `[lint.rules.SEC004].anon_roles`; the role graph is read live, so on an offline `--against` snapshot only a *leaking* policy scoped to a non-`{anon, PUBLIC}` role abstains **`UNVERIFIED`** rather than guess.
...
- **`reachability`** — every mode above reasons about a table's own policies. This one asks whether a **view** hands the rows back anyway. A `security_invoker = false` view executes as its **owner**, so the base table's RLS is evaluated against the owner rather than the caller: an anon-`SELECT`-able view owned by an RLS-exempt role returns *every row* while `--mode anon` correctly reports the table itself `PROVEN`. The gate is narrow, and each clause was measured on live PG16 — the owner must actually be exempt, meaning it owns the table and the table is **not** `FORCE`'d, or it holds `BYPASSRLS`. A view owned by an ordinary third role with a plain `SELECT` grant reads **zero** rows and is not flagged; `FORCE` on the base table shuts the owning-view case off; `BYPASSRLS` still bypasses *under* `FORCE`. Composes the `anon` verdict with view reachability the way `escalation` composes `cross-tenant` with owner reachability, so a table that already leaks everything to anon cedes to `--mode anon` rather than double-reporting. Regular views only — a materialized view stores rows captured at refresh, a different mechanism covered by `SEC053`/`SEC054`/`VIEW003`.
