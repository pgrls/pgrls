...
- **`reachability`** — every mode above reasons about a table's own policies. This one asks whether a **view** hands the rows back anyway. A `security_invoker = false` view executes as its **owner**, so the base table's RLS is evaluated against the owner rather than the caller: an anon-`SELECT`-able view owned by an RLS-exempt role returns *every row* while `--mode anon` correctly reports the table itself `PROVEN`. The gate is narrow, and each clause was measured on live PG16. Paths are walked hop by hop (`view → view → table`): the table's RLS runs as the owner of the *nearest enclosing* `security_invoker = false` view on the path — an invoker-ON outer view over an invoker-OFF inner view still leaks; a chain of invoker-ON views only is not flagged. That effective owner must be exempt — superuser/`BYPASSRLS` (bypasses even under `FORCE`), or, with the table **not** `FORCE`'d, the table owner or an `INHERIT` *member* of it (Postgres's `has_privs_of_role`; a `NOINHERIT` member is not owner-equivalent and is not flagged) — **or** the table's own policies must grant that owner every row under the anonymous auth context (`TO app_role USING (true)`: a definer view owned by a service role launders those rows). A view owned by an unrelated role with a plain `SELECT` grant reads **zero** rows and is not flagged; `FORCE` on the base table shuts the ownership case off; a hop the effective owner cannot `SELECT` is a dead path. "Anon can open the view" counts a table- *or column-level* `SELECT` grant to `anon`/`PUBLIC` or to any role anon is a member of. When the role-membership graph is unavailable (a pre-v26 snapshot, an offline `--sql-file` source) and the answer turns on membership, the table is **`UNVERIFIED`** rather than silently passed. Composes the `anon` verdict with view reachability the way `escalation` composes `cross-tenant` with owner reachability, so a table that already leaks everything to anon cedes to `--mode anon` rather than double-reporting. Regular views only — a materialized view stores rows captured at refresh, a different mechanism covered by `SEC053`/`SEC054`/`VIEW003`.
